...
class _SettingsPageState extends State<SettingsPage> {
                ...
                if (info.downloadUrl.isNotEmpty)
                  ...
                  OutlinedButton.icon(
                    ...
                    onPressed:
                        () => launchUrl(
                          Uri.parse(
                            'https://github.com/2233admin/cicada/releases/latest',
                          ),
                        ),
                  ),
              ],
  ...
  Future<void> _testFeishuConnection() async {
    ...
    if (appId.isEmpty || secret.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('请填写 App ID 和 App Secret')));
      return;
    }
    ...
    if (result.success) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('✗ 连接失败: ${result.error}')));
    }
  }
